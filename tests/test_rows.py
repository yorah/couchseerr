...
class RecordingClient(object):
    ...
    def season(self, tmdb_id, season_number, language=None):
        """Mirrors SeerrClient.season, which is itself a thin wrapper over get()."""
        return self.get(
...
def test_concurrent_hydration_keeps_the_payload_order(fixture, tmp_path):
    ...
    names. Here the first item submitted blocks until the last one has finished, so it
    completes after the item submitted last, and only consuming the futures in
    submission order still yields the payload's order.
    ...
    The Event orders ids[0] against ids[-1] and nothing else, so the middle items are
    free to land anywhere and the assertion has to be written about the pair the Event
    actually constrains. Asserting on the last slot instead reads as stricter but is
    merely luckier: a middle item scheduled late lands there, giving
    completed == [105, 103, 104], and the assertion fails on a property the test never
    meant to pin.
    """
    import threading
    ...
    def get(path, params=None):
        ...
        # Set after appending, never before: setting first releases the ids[0] worker to
        # race this one for its own slot, and the recorded order is then not the order
        # the Event was meant to impose.
        if tmdb_id == ids[-1]:
            last_finished.set()
        return detail[tmdb_id]
    ...
    assert completed.index(ids[0]) > completed.index(ids[-1]), \
        "the skew did not take; ordering is untested"
...
def test_season_is_fetched_and_cached(tmp_path):
    client = RecordingClient({"/tv/96677/season/1": {"episodes": []}})
    service = make_service(client, tmp_path)
    assert service.season(96677, 1) == {"episodes": []}
    assert service.season(96677, 1) == {"episodes": []}
    assert len(client.calls) == 1


def test_a_cached_season_is_served_without_a_call(tmp_path):
    client = RecordingClient({})
    service = make_service(client, tmp_path)
    service.cache.set(service._season_key(96677, 2), {"episodes": [1]}, TTL_DISCOVER)
    assert service.season(96677, 2) == {"episodes": [1]}
    assert client.calls == []


def test_two_seasons_of_one_show_cache_apart(tmp_path):
    client = RecordingClient({
        "/tv/96677/season/1": {"episodes": ["a"]},
        "/tv/96677/season/2": {"episodes": ["b"]},
    })
    service = make_service(client, tmp_path)
    assert service.season(96677, 1) != service.season(96677, 2)
    assert len(client.calls) == 2


def test_season_language_is_passed_to_the_client(tmp_path):
    client = RecordingClient({"/tv/96677/season/1": {"episodes": []}})
    service = make_service(client, tmp_path, language="fr")
    service.season(96677, 1)
    assert client.calls[0][1]["language"] == "fr"


def test_season_blank_language_is_omitted(tmp_path):
    client = RecordingClient({"/tv/96677/season/1": {"episodes": []}})
    service = make_service(client, tmp_path, language="")
    service.season(96677, 1)
    assert not client.calls[0][1].get("language")
