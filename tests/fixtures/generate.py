...
# --- /tv/{id}/season/{n}: one season's episodes -------------------------------------
# Shape measured against seerr 3.4.1 on 2026-08-24: no episodeCount at season level,
# externalIds present, and stillPath is an ABSOLUTE url rather than a bare path.
STILL_BASE = "https://image.tmdb.org/t/p/original/"


def tv_season(season_number, name, episodes, show_id=90100):
    """seerr's /tv/{id}/season/{n}. `episodes` is a list of
    (number, title, air_date, has_still)."""
    return {
        "id": 3500 + season_number,
        "airDate": episodes[0][2] if episodes else None,
        "name": name,
        "overview": "",
        "posterPath": "/s{0}.jpg".format(season_number),
        "seasonNumber": season_number,
        "externalIds": {"tvdbId": 800000 + show_id},
        "episodes": [
            {
                "id": 40000 + season_number * 100 + number,
                "airDate": air_date,
                ...
                "seasonNumber": season_number,
                "showId": show_id,
                # Absolute when TMDb has a still; null for an unaired episode.
                "stillPath": (
                    "{0}/still-s{1}e{2}.jpg".format(STILL_BASE, season_number, number)
                    if has_still else None
                ),
                "voteAverage": 7.5,
                "voteCount": 100,
            }
            for number, title, air_date, has_still in episodes
        ],
    }


write("tv_season_1", tv_season(1, "Season 1", [
    (1, "Carrier Wave", "2019-11-12", True),
    (2, "Dead Air", "2019-11-19", True),
]))
# Two aired, two still to come, the unaired pair carrying null stills and placeholder
# names exactly as measured. This is the case the window exists to show.
write("tv_season_partial", tv_season(2, "Season 2", [
    (1, "Groundwave", "2020-10-30", True),
    (2, "Skywave", "2020-11-06", True),
    (3, "Episode 3", "2026-11-04", False),
    (4, "Episode 4", "2026-11-11", False),
]))
# Not an empty season: seerr answers 500 for a season it cannot retrieve.
write("tv_season_missing", {"message": "Unable to retrieve season."})

#: Kodi VideoLibrary.GetEpisodes records, not a seerr payload: the detail window's season
#: view joins seerr's seasons against Kodi's own episodes, so the suite needs both shapes.
