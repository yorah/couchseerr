...
def test_refusal_carries_the_state_it_refused_on():
    """The message stays English for the log, but the UI must not read it: this module
    is pure and cannot resolve a localised label, so it hands the caller the state and
    lets the Kodi layer choose the wording."""
    client = FakeClient()
    with pytest.raises(RequestRefused) as excinfo:
        send_request(client, _settings(), "movie", 1, TileState.DOWNLOADING)
    assert excinfo.value.state is TileState.DOWNLOADING


def test_every_refusable_state_is_carried_back():
    """Derived from TileState, so a new member joins without editing this test. A state
    that arrived with no `.state` set would silently fall back to English wording."""
    for state in TileState:
        if state == TileState.ACTIONABLE:
            continue
        with pytest.raises(RequestRefused) as excinfo:
            send_request(FakeClient(), _settings(), "movie", 1, state)
        assert excinfo.value.state is state, state


def test_a_season_refusal_carries_the_season_state():
    client = FakeClient()
    with pytest.raises(RequestRefused) as excinfo:
        send_request(client, _settings(), "tv", 82856, TileState.OWNED, seasons=[1])
    assert excinfo.value.state is TileState.OWNED


def test_a_refusal_raised_without_a_state_still_works():
    """RequestRefused is part of the public surface -- a skin or a caller with no tile
    state can raise or catch it -- so the state is optional and defaults to None."""
    assert RequestRefused("movie/1 is already downloading").state is None
