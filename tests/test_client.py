...
def test_season_requests_the_season_endpoint():
    calls = []
    client = SeerrClient(
        "http://seerr.test:5055", "k", transport=make_transport(200, {}, calls)
    )
    client.season(96677, 1)
    assert calls[0][0] == "http://seerr.test:5055/api/v1/tv/96677/season/1"


def test_season_returns_the_payload_untouched(fixture):
    payload = fixture("tv_season_1")
    client = SeerrClient(
        "http://seerr.test:5055", "k", transport=make_transport(200, payload)
    )
    assert client.season(96677, 1) == payload


def test_a_season_seerr_cannot_retrieve_raises(fixture):
    client = SeerrClient(
        "http://seerr.test:5055", "k",
        transport=make_transport(500, fixture("tv_season_missing")),
    )
    with pytest.raises(SeerrRequestError):
        client.season(96677, 9)


def test_season_language_reaches_the_request():
    calls = []
    client = SeerrClient(
        "http://seerr.test:5055", "k", transport=make_transport(200, {}, calls)
    )
    client.season(96677, 1, language="fr")
    assert "language=fr" in calls[0][0]


def test_season_omits_a_blank_language():
    calls = []
    client = SeerrClient(
        "http://seerr.test:5055", "k", transport=make_transport(200, {}, calls)
    )
    client.season(96677, 1, language="")
    assert "language" not in calls[0][0]
