...
class DownloadProgress(object):
    ...
    def is_unreadable(self):
        ...
        populated `downloadStatus` shape has never been observed live - its field
        names come from seerr's source, not from measurement - so this is the one
        signal that must not be allowed to silently render as a real percentage.
        """
...
class Season(object):
    """One season of a show, as the detail window's season list needs it.
...
class Episode(object):
    """One episode as seerr describes it. Carries no ownership: that is Kodi's answer,
    joined on in seasons.join_episodes, because seerr does not know what is on this box.
    """

    __slots__ = ("number", "season", "title", "overview", "air_date", "still_path")

    def __init__(self, number, season, title, overview="", air_date=None,
                 still_path=None):
        self.number = number
        self.season = season
        self.title = title
...
def parse_season_episodes(raw):
    """Episodes of one season, ascending by number.

    An entry with no usable `episodeNumber` is dropped: it can be neither ordered nor
    joined against Kodi, so it could only ever render as a row in an arbitrary place.
    `_parse_date` already returns None for anything it cannot read, so a bad airDate
    degrades to "no date" rather than taking the season down.
    """
    episodes = []
    for entry in (raw.get("episodes") or []):
        ...
        episodes.append(
            Episode(
                ...
                air_date=_parse_date(entry.get("airDate")),
                still_path=entry.get("stillPath"),
            )
        )
