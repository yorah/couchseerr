...
Kodi does not reliably store a tmdb uniqueid, which is what makes this a join rather than
a lookup. Measured on the target library: 559 of 560 movies carry imdb only, 108 of 109
shows carry tvdb only, and none carry tmdb. seerr's detail payload supplies both, so the
join goes through imdb or tvdb, with exact title-and-year as the only fallback.
"""
