...
class Row(object):
    #: `label_key`, never a display string: this module is pure, so any English it held
    #: could only ever reach the screen untranslated. The Kodi layer resolves the key
    #: against LABEL_IDS when it builds the root listing.
    __slots__ = ("key", "label_key", "path", "content_type")

    def __init__(self, key, label_key, path, content_type):
        self.key = key
...
ROWS = {
    "trending": Row("trending", "row_trending", "/discover/trending", "movies"),
    "upcoming_movies": Row(
        "upcoming_movies", "row_upcoming_movies", "/discover/movies/upcoming", "movies"
    ),
    "popular_tv": Row("popular_tv", "row_popular_tv", "/discover/tv", "tvshows"),
    "processing": Row("processing", "row_processing", "/media", "videos"),
}
...
class RowService(object):
    def __init__(self, client, cache, base_url, image_base, language, today_provider,
                 labels=None, max_workers=4, widget=False):
        self.client = client
        self.cache = cache
        ...
        # Passed straight through to build_spec, which decides what it changes about a
        # tile. Set by the Kodi layer, which is the only place that can see whether this
        # listing is being rendered into a home widget (routes._in_widget).
        self.widget = widget
    ...
    def _spec(self, item):
        ...
        return build_spec(item, state, self.base_url, self.image_base,
                          self.labels, self.widget)
    ...
    def _season_key(self, tmdb_id, season_number):
        """Mirrors _detail_key's own spelling: season(), and nothing else, must agree
        with itself, so there is only one format to keep consistent."""
        return "season:{0}:{1}:{2}".format(tmdb_id, season_number, self.language)

    def season(self, tmdb_id, season_number):
        """One season's raw payload, cached at TTL_DISCOVER like the detail payload.

        The window opens and closes a season on every step in and out of it, so an
        uncached fetch here is a network round trip per keypress on the slowest device
        this targets. There is deliberately no invalidate_season: a request changes the
        season's *state*, which comes from the detail payload _do_request already
        invalidates, not from this one.
        """
        cache_key = self._season_key(tmdb_id, season_number)
        payload = self.cache.get(cache_key)
        if payload is None:
            payload = self.client.season(tmdb_id, season_number, self.language or None)
            self.cache.set(cache_key, payload, TTL_DISCOVER)
        return payload
